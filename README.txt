INTRODUCTION
============
   This is demonstration of integration of IBM Spectrum Scale with IBM Watson Cognitive services hosted at IBM BLuemix. This module utilizes Openstack Swift middlewares at proxy middleware chain and intercepts the request to check if the request satisfies the criteria for cognitive processing. If such request is selected then it is added to external STOMP(Streaming Text Oriented Messaging Protocol http://stomp.github.io/) based queue for further processing. 
...
==========
   a) Prerequisites:
      - IBM Bluemix Visual Recognition API Key from https://console.ng.bluemix.net/catalog/services/visual-recognition
   ...
   b) Copy and install rpm - dist/watsonintegration-0.1-1.noarch.rpm on all the protocol nodes.

   c) Start Apache Active MQ, on all protocol nodes
...
=======================
   a) Update /etc/swift/watson-integration.conf, on all protocol nodes
   ...
   b) Update /etc/swift/proxy-server.conf, on *any* protocol nodes
   ...
   c) Start watsonintegration service on all protcol nodes.
